...
pub           = rospy.Publisher('/ardrone/states', States, queue_size = 200)
pub_acc       = rospy.Publisher('/pen_input', PointStamped, queue_size = 200)
...
def Driver():
    ...
    states.u      = (states.u     + dy[3,0]/rate)#+np.random.normal(0,.001,1)[0]
    states.v      = (states.v     + dy[4,0]/rate)#+np.random.normal(0,.001,1)[0]
    states.w      = (states.w     + dy[5,0]/rate)#+np.random.normal(0,.0001,1)[0]
    states.phi    = 180*(states.phi*PI/180   + dy[6,0]/rate)/PI #+np.random.normal(0,.001,1)[0]
    states.theta  = 180*(states.theta*PI/180 + dy[7,0]/rate)/PI #+np.random.normal(0,.001,1)[0]
    states.psi    = 180*(states.psi*PI/180   + dy[8,0]/rate)/PI #+np.random.normal(0,.0001,1)[0]
...
#             rospy.sleep(.5)
    ...
    states.name = "Simulated AR.Drone"
    states.visible = True
