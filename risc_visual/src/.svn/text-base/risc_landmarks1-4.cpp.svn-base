...
/*======================================================
	...
	File name: 	risc_landmarks1-4.cpp
...
image_transport::Publisher im3;
image_transport::Publisher im4;
risc_msgs::Observed_rois R;
risc_msgs::Risc_rois roi[4];
const char* const names[] = {"risc1","risc2","risc3","risc4"};
...
void imageCallback1(const sensor_msgs::ImageConstPtr& original_image)
{
	...
	if ( green_contours.size() != 0 )
	{
		for( int i = 0; i< green_contours.size(); i++ )
		{
			...
			{ 
			   ...
			   /*================================================
				*    Store the idx and the corresponding Mask
				===============================================*/
...
//	cv::waitKey(3);                                                   
        ...
        seen.landmarks.resize(4);
	seen.landmarks[0] = green_roi;
	seen.landmarks[1] = pink_roi;
	seen.landmarks[2] = orange_roi;
	seen.landmarks[3] = blue_roi;
        int length = 0;
        R.Obj.resize(0+1);
	for(int i; i<4; i++)
	{
		seen.landmarks[i].visible = false;
		if(seen.landmarks[i].width != 0 && seen.landmarks[i].height != 0 ) 
		{seen.landmarks[i].visible = true;
         length++;
         R.Obj[0].landmarks.resize(length);
         R.Obj[0].landmarks[length-1] = seen.landmarks[i];}
	}

	R.Obj[0].name = names[0];
	R.header.stamp = time;
...
//	cv::waitKey(3);                                                   
	...
	for(int i; i<4; i++)
	{
		...
		{seen.landmarks[i].visible = true;
         length++;
         R.Obj[1].landmarks.resize(length);
         R.Obj[1].landmarks[length-1] = seen.landmarks[i];}
	}

	R.Obj[1].name = names[1];
	R.header.stamp = time;
...
void imageCallback3(const sensor_msgs::ImageConstPtr& original_image)
{
     ros::Time time = ros::Time::now();
...
//	cv::waitKey(3);                                                   
	...
	for(int i; i<4; i++)
	{
		...
		{seen.landmarks[i].visible = true;
         length++;
         R.Obj[2].landmarks.resize(length);
         R.Obj[2].landmarks[length-1] = seen.landmarks[i];}
	}

	R.Obj[2].name = names[2];
	R.header.stamp = time;

    cv_ptr->image = drawCross(cv_ptr->image);
    
    im3.publish(cv_ptr->toImageMsg());
...
void imageCallback4(const sensor_msgs::ImageConstPtr& original_image)
{
     ros::Time time = ros::Time::now();
...
//	cv::waitKey(3);                                                   
	...
	for(int i; i<4; i++)
	{
		...
		{seen.landmarks[i].visible = true;
         length++;
         R.Obj[3].landmarks.resize(length);
         R.Obj[3].landmarks[length-1] = seen.landmarks[i];}
	}

	R.Obj[3].name = names[3];
	R.header.stamp = time;

    cv_ptr->image = drawCross(cv_ptr->image);

    im4.publish(cv_ptr->toImageMsg());
